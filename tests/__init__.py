...
class IMATestCase(unittest.TestCase):
    ...
    @classmethod
    def eval(cls, tests: Optional[str] = None, verbosity: int = 2, **params: dict[str, Any]) -> unittest.TestResult:
    ...
    def assertArraysClose(
        ...
        """
        Is shape sensitive
        Is *not* dtype sensitive
        """
        try:
            err_str = ''
            np.testing.assert_allclose(actual, expected, rtol=rtol, atol=atol, verbose=True)  # shape sens., dtype ins.
    ...
    def assertArraysEqual(
        ...
        """
        Is sensitive to both shape and dtype
        """
        self.assertEqual(actual.dtype, expected.dtype)
        try:
            err_str = ''
            np.testing.assert_array_equal(actual, expected, verbose=True)  # shape sensitive, dtype insensitive
        except AssertionError as e:
            err_str = f'{e}\n'
        if err_str:
            self.fail(msg=f'{msg}\n{err_str}')
